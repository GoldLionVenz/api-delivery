...
exports.default = makeIncrementProductShoppingCart;

function makeIncrementProductShoppingCart({
  shoppingCartModel,
  getShoppingCartResponse
}) {
  return async function incrementProductShoppingCart({
    user,
    ...productInfo
  } = {}) {
    ...
    const productFind = cart.items.find(item => item._id == productInfo.item);
