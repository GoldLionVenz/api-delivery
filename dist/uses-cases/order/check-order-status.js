...
function makeCheckOrderStatus({
  paymentGateWay,
  orderModel,
  now
}) {
  return async function checkOrderStatus({
    orderId,
    paymentRequest
  } = {}) {
    ...
    const execute = await paymentGateWay.executePayment(paymentRequest.paymentId, excecuteData);
