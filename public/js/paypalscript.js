...
async function getShoppingCart({ token, shippingName, shippingAdrress }) {
  window.postMessage(
    JSON.stringify({ type: "loading", state: { loading: true } })
  );
  ...
  window.postMessage(
    JSON.stringify({ type: "loading", state: { loading: false } })
  );
  const resp = await peticion.json();
  if (peticion.status === 200) {
    document.getElementById(
      "price"
    ).innerHTML = `Total a pagar $ ${formatCurrency(
  ...
  paypal
    .Buttons({
      fundingSource:
        button === "paypal" ? paypal.FUNDING.PAYPAL : paypal.FUNDING.CARD,
